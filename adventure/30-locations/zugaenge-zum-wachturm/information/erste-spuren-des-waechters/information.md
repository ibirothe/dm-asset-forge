---
id: info-erste-spuren-des-waechters
type: information
title: "Erste Spuren des Wächters"
status: draft
version: 4
scope: local
truth_status: established
confidence: supported
accessibility: limited
primary_location: "loc-zugaenge-zum-wachturm"
discovery_locations: ["loc-zugaenge-zum-wachturm", "loc-alter-wachturm"]
known_by: ["npc-waechter-der-glocke"]
related_threads: ["plot-fluch-oder-gier"]
tags: [guardian, warning, route-clue]
themes: [threshold, warning]
created: 2026-08-14
updated: 2026-08-29
---

# Erste Spuren des Wächters

## Statement

Auf der verlassenen Pilgerstraße und am Kanalrand gibt es frühe Zeichen, dass eine gebundene Wächtergestalt den Weg zur Glocke markiert und auf Raubabsicht reagiert.

## Truth and limits

Etabliert. Die Spuren beweisen die Existenz des Wächters, aber noch nicht, ob er Feind, Prüfer, Opfer oder möglicher Verhandlungspartner ist.

## Discovery points

<!-- Orte, Akteure, Objects, Handouts oder Situationen relativ verlinken und pro Pfad nur Zugang und Präsentation beschreiben. Die Aussage selbst bleibt ausschließlich unter Statement kanonisch. -->

- Primary location: [Zugänge zum Wachturm](../../location.md)
- Grenzstein an der Pilgerstraße: Siegelreste, tiefe Kratzspuren und kalter Ruß.
- [Unwetter am Waldpfad](../../scenes/unwetter-am-waldpfad/scene.md): trockene Siegelspuren im Starkregen, eine reglose Gestalt zwischen den Bäumen und metallisches Dröhnen begleiten den Aufstieg nach Norden.
- Kanalstrecke: ähnliche Zeichen an einer trockenen Nische, obwohl dort Feuchtigkeit alles andere verwischt.
- Nach dem [Kleinen Kampf an der Pilgerstraße](../../encounters/kleiner-kampf-an-der-pilgerstrasse/encounter.md): Der Standard-Wachposten beschreibt trockene schwarze Siegel, kaltes Metallgefühl und Dröhnen; beim Grenzstein werden dieselben Zeichen unmittelbar sichtbar. Optionale Gegnervarianten verändern nur die Aussage, nicht das Zeichen.
- [Alter Wachturm](../../../alter-wachturm/location.md): Die gleichen Zeichen erscheinen in Eingangshalle, Treppenaufgang oder Siegelkammer wieder.

## Preconditions

Waldweg oder Kanal aktiv untersuchen, nach Spuren suchen oder nach dem Kampf nicht sofort weiterhetzen.

## Presentation clues

Ein eingeritztes Wächterzeichen, kaltes Metallgefühl ohne Metallquelle, Bewegungen im Augenwinkel, im Regen trocken bleibende Siegelspuren und der Eindruck, dass der Weg nicht nur verlassen, sondern bewacht ist.

## Interpretation risks

Die Gruppe kann den Wächter vorschnell als Dämon deuten oder den Hinweis ignorieren, wenn der Kampf die Aufmerksamkeit bindet.

## Consequences when learned

Die Gruppe kann im Turm nach Verhandlung, Opfer, Siegelkammer oder Umgehung suchen, statt den Wächter erst im Finale als bloßen Gegner zu behandeln.

## Consequences when missed

Der Wächter erscheint im Turm überraschender und die Gruppe verliert frühe Hebel für eine nicht reine Kampflösung.

## Related assets

- [Wächter der Glocke](../../../alter-wachturm/npcs/waechter-der-glocke/npc.md)
- [Fluch oder Gier](../../../../20-plot/threads/fluch-oder-gier/plot-thread.md)
