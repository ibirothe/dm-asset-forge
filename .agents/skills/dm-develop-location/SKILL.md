...
1. Read the root `AGENTS.md`, the normative `docs/asset-katalog.md`, `docs/asset-authoring-guide.md`, `docs/metadaten-und-werte.md`, and `docs/beziehungen-und-speicherorte.md`, `adventure/README.md`, `adventure/50-indexes/locations.md`, and the target location if it exists.
...
4. Apply the `location` section of the authoring guide. Give the location a table purpose, immediate impression, sensory identity, access boundaries, usable areas, pressures, and possible change over time. Set qualitative metadata only with documented values.
...
7. Apply the matching guide section to every local asset. Keep it `draft` until its type-specific Definition of Done is met.
