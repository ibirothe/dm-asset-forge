...
9. Inspect the navigation files reported by `scripts/new_asset.py`. Preserve its idempotent Location index and deterministic owner or Parent links; add only missing contextual cross-links and never duplicate an existing target. Update metadata logs and any other affected indexes.
