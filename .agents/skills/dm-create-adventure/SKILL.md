...
14. Do not generate images during initialization. A requested Player Character may be created as a canonical draft, but do not create its `player.md` without presenting and obtaining approval for the exact release draft.
15. Check every item in the intake Definition of Done. Run `python3 scripts/validate_adventure.py` and fix structural errors.
16. Return the German completion summary in the exact content order defined by the intake workflow: created state, confirmed basis, assumptions, open questions, validation, and prioritized next steps.
