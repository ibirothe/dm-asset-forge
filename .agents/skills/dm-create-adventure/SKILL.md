...
18. Update `60-session/readiness-report.md` from checks actually completed: keep the audit result `not-run` until an audit exists, record only results, dates, source links, blockers, and exactly one next action, and never set the overall status to `ready` unless preflight is `ready`, validation passed, the audit is clear, and no blocker remains. Rerun the validator after changing the report.
19. Return the German completion summary in the exact content order defined by the intake workflow: created state, confirmed basis, assumptions, open questions, validation, and prioritized next steps.
