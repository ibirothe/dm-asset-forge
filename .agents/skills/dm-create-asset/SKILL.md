...
description: Create or revise one of the 15 structured, system-neutral Dungeon Master asset types in an existing one-shot. Use for focused asset work; use the location skill when the request centers on developing a whole place.
---

# Create or revise an asset

1. Always read `AGENTS.md`, `docs/asset-katalog.md`, the selected type section of `docs/asset-authoring-guide.md`, the target adventure overview, the relevant index, and the minimum linked context. For Information work also read `adventure/50-indexes/clue-matrix.md`. Do not load unrelated guide sections.
2. Load additional references only when the task requires them: `docs/metadaten-und-werte.md` when changing frontmatter or qualitative values; `docs/beziehungen-und-speicherorte.md` when canonical path, ownership, relations, or lifecycle are involved; `docs/adventure-structure-guide.md` for `scene`, `information`, `encounter`, `plot-thread`, `event`, `npc`, `player-character`, or `faction` work involving choices, consequences, agency, or information paths; `docs/player-handout-workflow.md` before creating or replacing a Handout's `player.md`; `docs/player-character-workflow.md` before creating or replacing a Player Character's `player.md`; and `docs/bild-workflow.md` before Visual-brief or PNG work.
...
4. Create any of the 15 catalog types with the asset generator:
...
8. Keep `status: draft` while a shared or type-specific Definition of Done criterion is missing. Set `ready` only after checking every criterion; template completeness alone is insufficient.
   ...
   For a player-facing Player Character file, present the complete proposed `player.md`, preserve every open character decision, run the documented Safety-Check, and obtain the user's explicit approval before writing or replacing it.
...
9. Keep the full asset in exactly one canonical file. Inspect the navigation files reported by `scripts/new_asset.py`: it maintains the applicable index and deterministic owner, parent, or Subject links without duplicates and preserves an authored table-context cell on overwrite. For a new Information asset it also adds one `open` starter row to the global clue matrix; replace that row with one concrete row per discovery path, using stable-ID links and without copying statement or truth status. Replace a placeholder table context with one short action-relevant summary, update it when the asset's immediate table function changes, and keep the row's only link pointed at the canonical asset. If the asset is central, update its direct README route or confirm that one matching index provides the route. Add only missing contextual links for appearance locations and other structured relationships, and update related assets wherever the relationship model requires a reciprocal link. Never add a second index row or duplicate an existing clue path.
