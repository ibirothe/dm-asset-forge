...
1. Read `AGENTS.md`, the normative `docs/asset-katalog.md`, `docs/asset-authoring-guide.md`, `docs/metadaten-und-werte.md`, and `docs/beziehungen-und-speicherorte.md`, the target adventure overview, the relevant index, and the minimum linked context.
...
5. Use only documented keys and controlled values. Represent unknown, inapplicable, empty-list, and open values according to the metadata specification instead of fabricating unsupported canon.
6. Keep `status: draft` while a shared or type-specific Definition of Done criterion is missing. Set `ready` only after checking every criterion; template completeness alone is insufficient.
7. Keep the full asset in exactly one canonical file. Add relative links from its owner, appearance locations, and index below `adventure/`. Update related assets wherever the relationship model requires a reciprocal link.
8. Update `90-meta/change-log.md`; record assumptions and questions in their respective files.
9. Run the adventure validator and summarize the result.
